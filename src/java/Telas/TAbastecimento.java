...
@WebServlet(name = "TAbastecimento", urlPatterns = {"/TAbastecimento"})
public class TAbastecimento extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            opcao = " WHERE Data BETWEEN '"+ ano +"-"+ mes +"-01' AND '"+ ano +"-"+ mes +"-30' AND Categoria = 0 GROUP BY VeiculoCombustivel";

            List<VFrDiariaCategoria> vFrDiariaCategorias = new IDiaria().DiariaCategoriaView(opcao, true);
            
            if (vFrDiariaCategorias.size() > 0){
                out.println("<form action='TAbastecimento'>");
                out.println("<table class='Pesquisa' width=100% boder=0>");
                out.println("   <tr><td colspan=10 style='font-size: 17px;'>Controle de Abastecimento da Prefeitura/Câmara Municipal de <br>"+ prefeitura +"</td></tr>");
                out.println("   <tr><th colspan=10></th><tr>");
                out.println("   <tr>");
                out.println("       <th width='10%'>Exercício</th><td width='10%'>"+ ano +"</td><th width='10%'>Mês de Referência</th><td width='10%'>"+ iMenu.MesDescricao(Integer.parseInt(mes))+"</td><td rowspan=2>");
                if (request.getParameter("Imprimir") != null && !request.getParameter("Imprimir").isEmpty()){
                ...
                out.println("<table class='Pesquisa' width=50% boder=0>");
                out.println("   <tr>");
                out.println("       <th>Combustível</th>");
                out.println("       <th>Quantidade Adquirida</th>");
                ...
                out.println("<tr><th colspan=10></th><tr>");
                out.println("<tr><td colspan=10 style='text-align:left;'>a- Quantidade de combustível adquirida no mês.<td></tr>");
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(TAbastecimento.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
